...
\item The internal function \texttt{getCmatrix} is updated with an optional argument, \texttt{by.gene = TRUE}, to consider pathways at the gene-level instead of the probeset-level.
