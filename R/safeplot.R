...
function(safe = NULL, cat.name = NULL, c.vec = NULL, local.stats = NULL, p.val = NULL, 
                     one.sided = FALSE, limits = c(-Inf,Inf), extreme = NA, italic = FALSE , 
    ...
    if(!is.null(safe)){
      ...
      if (prod(limits == c(-Inf,Inf)) == 1) {
        ...
        limits[2] <- min(local.stats[(safe@local.pval <= 0.05)&(local.stats > 0)])
      }
      c.vec <- safe@C.mat[,dimnames(safe@C.mat)[[2]] == cat.name]
      p.val <- safe@global.pval[dimnames(safe@C.mat)[[2]] == cat.name]
    }
...
#    if(names(dev.cur()) %in% c("windows","null device")) x11(width=8,height=5.5)
    ...
    if (one.sided){
      if (extreme) c.vec.sorted[local.sorted<limits[2]] <- 0
      tick<-(1:m)[c.vec.sorted==1]
      g2 <- length(tick)
      tick.adj<-rep(min(tick[g2],0.99*m),g2)
      lines(c(tick.adj[g2]+0.002*m,tick[g2]), c(0.25, 0.1))
      i <- g2;  stop <- 0
      while(stop == 0){
        i <- i - 1
        tick.adj[i]<-min(tick.adj[i+1]-0.02*m,tick[i])
        if(tick.adj[i]>0) lines(c(tick.adj[i],tick[i]), c(0.25, 0.1)) else {
          stop <- i+1}
        if(i == 1) stop <- 1
      }
      text(tick.adj[stop:g2]+0.0005*m,.26,gene.names.sorted[c.vec.sorted==1][stop:g2],
      ...
      if (gB) while(stopB == 0){
        i <- i - 1
        tick.adj[i]<-min(tick.adj[i+1]-0.02*m,tick[i])
        if(tick.adj[i]>zero) lines(c(tick.adj[i],tick[i]), c(0.25, 0.1)) else {
          stopB <- i+1}
